...
function createExercise(exercise, index) {
  return {
    tempId: `${exercise.id ?? Date.now()}-${index}`,
    order: index + 1,
    dayOfWeek: exercise.dayOfWeek ?? '',
    exerciseName: exercise.exerciseName ?? '',
    series: exercise.series ?? '',
    amount: exercise.amount ?? '',
    unit: exercise.unit ?? 'repeticiones',
  ...
  useEffect(() => {
    async function loadRoutine() {
      try {
        ...
        const data = await getRoutinesByClient(id)
        const routine = data.currentRoutine

        if (
          !routine ||
          String(routine.id) !== String(routineId)
        ) {
          throw new Error(
            'Solo puede editarse la rutina activa',
          )
        }
...
export default AdminEditRoutinePage
